  ...
  private:
			  ...
			  struct mg_connection *conn)
	{
		...
		if (CivetServer::getParam(conn, "param", s))
		{
			...
			mqtt_publish("test/shixq", s.c_str(), 1);
		}
		else
		{
...
void mqtt_message_callback(const struct mosquitto_message *message)
{
	...
	int rc = unpack_message((char *)message->payload, len, mm);
...
int testrequest()
{
	printf("test request start\n");
	/* connection object */
	struct mg_connection *conn;
	char ebuf[1024] = {0};
	const char *host = "localhost";
	const int port = 8088;
	struct mg_client_options options;
	options.host = host;
	options.port = port;
	options.client_cert = "/home/shixq/mosquitto/certificate/client.pem";
	conn = mg_connect_client_secure(&options, ebuf, sizeof(ebuf));
	...
	if (conn)
	{
		...
		/* Send the header and terminate the header with an empty line */
		mg_printf(conn, "GET /v1alpha/iceconfig HTTP/1.1\r\n");
		mg_printf(conn, "Host: localhost:8088\r\n");
		mg_printf(conn, "User-Agent: Mozilla/5.0 (X11; Linux x86_64…) Gecko/20100101 Firefox/62.0\r\n");
		mg_printf(conn, "\r\n");

		/* While there are chunks of data that need to be sent from the
   		client to the server, send chunk sizes and chunks. */
		// while ((chunk = GetNextChunk()) != NULL)
		// {
		// 	mg_printf(conn, "%x\r\n%s\r\n", strlen(chunk), chunk);
		// }
		/* Terminate the body data send from client to server */
		// mg_printf(conn, "0\r\n\r\n");

		/* Now get the response from the server */
		ret = mg_get_response(conn, ebuf, sizeof(ebuf), 1000);
		if (ret >= 0)
		{
			const struct mg_response_info *ri = mg_get_response_info(conn);

			printf("Response info: %i %s\n",
				   ri->status_code,
				   ri->status_text);
		...
		else
		{
			...
			printf("Got no response from %s:\n%s\n", host, ebuf);
		}
		printf("close the connection %s\n", host);
		// fflush(stdout);
